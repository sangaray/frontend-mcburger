...
export function addReviews(id, newReviews) {
  axios
    .post(`http://localhost:3001/product/${id}/reviews/add`, newReviews)
    .then((data) => data)
    .catch((error) => {
      console.log(error);
    });
...
export function getProductExists(userId, productId) {
  return axios
    ...
    .then((orders) => {
      let orderPaid = orders.data.filter(
        (order) => order.status === "Procesando" || order.status === "Completa"
      );
      orderPaid = orderPaid[orderPaid.length - 1];
      return orderPaid;
    })
    .then((orderPaid) => {
      let prodBool = orderPaid.products.filter(
