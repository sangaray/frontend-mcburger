...
export default function SignInSide() {
  ...
  const handleSubmit = async (event) => {
    ...
    let json = await axios.post(
      "https://test-deploy-production-3b4b.up.railway.app/user/login",
      dat
    );
    console.log(json);
    if (json.data.token !== null) {
      document.cookie = `token=${json.data.token}; max-age=${
        60 * 1
      }; path=/; samesite=strict`;
      getAllUserCart(json.data.email)(dispatch);
  ...
  useEffect(() => {
    const autenticated = async () => {
      if (isAuthenticated && activeUser == false) {
        ...
        let json = await axios.post(
          "https://test-deploy-production-3b4b.up.railway.app/user",
          dat
        );
        document.cookie = `token=${json.data.token}; max-age=${
          60 * 1
  ...
  const handleClose = async () => {
    ...
    let userDate = await axios.post(
      "https://test-deploy-production-3b4b.up.railway.app/user/one",
      { email: userN.email }
    );
  ...
  const prueba = async (e) => {
    ...
    const test = await axios.post(
      "https://test-deploy-production-3b4b.up.railway.app/user/test",
      dat
    );
  ...
  const handleUpdate = async (e) => {
    ...
    const updateSend = await axios.put(
      "https://test-deploy-production-3b4b.up.railway.app/user",
      input
    );
  ...
  const updateUser = () => {
    let state;
    !userN.first_name || !userN.last_name || !userN.phone || !userN.address
      ? (state = true)
      : (state = false);
    if (state === true) {
      return (
        <>
          <Grid sx={{ marginTop: 0 }}>
            <Card
              style={{ maxWidth: 450, padding: "20px 5px", margin: "auto" }}
    ...
    <ThemeProvider theme={theme}>
      ...
      <Grid container component="main" sx={{ height: "100vh" }}>
        ...
        <Grid
          ...
          sx={{
            backgroundImage:
              "url(https://media.istockphoto.com/id/511484502/es/foto/hamburguesa-con-queso-y-tocino-con-queso-y.jpg?s=612x612&w=0&k=20&c=QHjjrvjpkfB_9Lsxhn5wC_JcZKJ4b_4blmA5bWgT-40=)",
        ...
        <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
          ...
          >
            <Avatar
              sx={{ m: 1, bgcolor: "black", width: 100, height: 100 }}
              src={logo}
            >
              {/*  <LockOutlinedIcon /> */}
            </Avatar>
      ...
      >
        <DialogContent>{updateUser()}</DialogContent>
      </Dialog>
    </ThemeProvider>
  );
}
