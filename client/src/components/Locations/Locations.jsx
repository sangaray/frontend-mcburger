      ...
      array.push(
        <Box>
          <LocationCards
            address={branch.address}
            locality={branch.locality}
            coordenates={branch.coordenadas}
          />
        </Box>
      );
    });
      ...
      <Box display={"flex"} justifyContent={"space-between"} bg={"#f5ebdc"}>
        <Box className="ubications">
          <Box>
            ...
            <Box> </Box>{" "}
            {/* <Input border={'solid #000000'} display={'block'} w={'500px'} backgroundColor="white" mt="10px" mb={'10px'} placeholder="Enter your location here..." /> */}
            <Text as={"b"} fontSize={"1.5rem"} mb={"20px"}>
              Restaurants
            </Text>
          </Box>
          <Box mt={"20px"}>{cardsGenerator()}</Box>
        </Box>
        <Box position={"fixed"} w={"1050px"} h={"650px"} ml={"36%"} mt={"25px"}>
          <MapView branches={branches} />
          <Button
            colorScheme={"orange"}
            onClick={() => handleFilterUserPosition()}
            mt={"10px"}
          >
            My position
          </Button>
